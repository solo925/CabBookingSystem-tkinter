...
class travel:
    ...
    #==========================================Define Functiom==================================================
        ...
        def iExit():
            iExit= ms.askyesno("Prompt!","Are you sure you want to exit?")
            if iExit > 0:
                root.destroy()
                return
        ...
        def Kilo():
            if var2.get() == 0:
                ...
                if varl1.get() == "Moi Avenue":
                    switch ={"Kimathi street": 10,"Mama Ngina": 8,"Tom Mboya":6,"Moi Avenue": 0}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Kimathi street":
                    switch ={"Kimathi street": 0,"Mama Ngina": 2,"Tom Mboya":5,"Moi Avenue": 10}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Mama Ngina":
                    switch ={"Kimathi street": 2,"Mama Ngina": 0,"Tom Mboya":3,"Moi Avenue": 8}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "Tom Mboya":
                    switch ={"Kimathi street": 5,"Mama Ngina": 3,"Tom Mboya":0,"Moi Avenue": 6}
                    Km.set(switch[varl2.get()])        
        ...
        def selectCar():
            ...
            if carType.get() == 1:
                ...
                Item5 = float(3000)
                FordMondeo.set("ksh "+ str(Item5))

        def Initiate_payment():
            phonen =Mobile.get()
            if phonen == Mobile.get():
                ms.showinfo("Info", f"please check your phone for phone number {phonen} to complete transaction")
            else:
                ms.showwarning("Error", "Invalid phone number")
        ...
        def Total_Paid():
            if ((var1.get() == 1 and var2.get() == 1 and var3.get() == 1 or var4.get() == 1) and carType.get() != 0 and journeyType.get() != 0 and (varl1.get() != "" and varl2.get() !="")):
                if journeyType.get()==1:
                    ...
                    Tax = "ksh " + str('%.2f'%((Cost_of_fare) *0.09))
                    ST = "ksh " + str('%.2f'%((Cost_of_fare)))
                    TT = "ksh" + str('%.2f'%(Cost_of_fare+((Cost_of_fare)*0.9)))
   ...
   #================================================mainframe========================================================================
        ...
        self.lblTitle=Label(Tops,font=('arial',40,'bold'),text="\t NDEDAS ONLINE CAR BOOKING SYSTEM")
        self.lblTitle.grid()
    ...
    #===============================================Cab Information==============================================================
        ...
        self.cboPickup['value']=('','Moi Avenue','Tom Mboya','Mama Ngina','Kimathi street')
        self.cboPickup.current(0)
        ...
        self.cboDrop['value']=('','Kimathi street','Mama Ngina','Tom Mboya','Moi Avenue')
        self.cboDrop.current(0)
    ...
    #======================================Button========================================================================================
        ...
        self.btnTotal = Button(ButtonFrame,padx=14,bd=7,font=('arial',11,'bold'),width = 2,text='Total',command=Total_Paid).grid(row=0,column=0)
        self.btnReceipt = Button(ButtonFrame,padx=14,bd=7,font=('arial',11,'bold'),width = 2,text='Receipt',command=Receiptt).grid(row=0,column=1)
        self.btnReset = Button(ButtonFrame,padx=14,bd=7,font=('arial',11,'bold'),width = 2,text='Reset',command=Reset).grid(row=0,column=2)
        self.btnPay = Button(ButtonFrame,padx=14,bd=7,font=('arial',11,'bold'),width = 2,text='pay',command=Initiate_payment).grid(row=0,column=3)
        self.btnExit = Button(ButtonFrame,padx=14,bd=7,font=('arial',11,'bold'),width = 2,text='Exit', command=iExit).grid(row=0,column=4)
